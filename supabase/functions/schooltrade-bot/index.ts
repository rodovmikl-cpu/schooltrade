  ...
  try {
    ...
    if (mode === "generate_image") {
      ...
      if (isUnsafeImagePrompt(prompt)) {
        return new Response(JSON.stringify({ error: "אני לא יכול לייצר תמונה עם תוכן כזה." }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (result.error || !result.imageUrl) {
        return new Response(JSON.stringify({ error: result.error || "לא הצלחתי ליצור תמונה" }), {
          status: result.status || 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      return new Response(JSON.stringify({ image: result.imageUrl, text: result.text }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // Server-side fallback: if last user message is clearly an image-generation request
    // (and not multimodal with an attached image), route to image generation instead of
    // letting the chat LLM possibly refuse.
    ...
    if (lastMsg && lastMsg.role === "user") {
      const lastText = typeof lastMsg.content === "string"
        ...
        : Array.isArray(lastMsg.content)
          ...
          : "";
      const hasAttachedImage = Array.isArray(lastMsg.content) &&
        lastMsg.content.some((c: any) => c?.type === "image_url");
      if (!hasAttachedImage) {
        const intent = detectImageIntent(lastText);
        if (intent && !isUnsafeImagePrompt(intent)) {
          const result = await generateImageWithRetry(intent, LOVABLE_API_KEY);
          if (result.imageUrl) {
            return new Response(JSON.stringify({ image: result.imageUrl, text: result.text || `מייצר עבורך תמונה של ${intent} 🎨`, autoGenerated: true }), {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
            });
          }
