...
const generateRound = (classLevel: number): ReadingRound => {
  // Simple English texts - much easier than Hebrew game
  const easyTexts = [
    "Tom has a cat. The cat is orange. Tom plays with his cat every day. They are happy together.",
    "Lisa likes apples. She eats an apple every morning. Apples are red and sweet. They are good for you.",
    "The sun is yellow. It shines in the sky. The sun gives us light. We need the sun to live."
  ];

  const mediumTexts = [
    "My family lives in a small house. We have a garden with flowers. My mom loves to plant roses. My dad likes to read in the garden.",
    "I go to school by bus. The bus is big and yellow. I sit with my friend Ben. We talk about games and movies.",
    "Dogs are good pets. They are friendly and fun. You need to walk your dog every day. Dogs love to play with balls."
  ];

  const hardTexts = [
    "The library is my favorite place. It has many books about different topics. I like to read stories about adventures. The librarian helps me find good books to read.",
    "Summer is the best season. We go to the beach and swim in the sea. I build sand castles with my sister. At night, we eat ice cream.",
    "My grandmother makes the best cookies. She uses chocolate chips and butter. The cookies smell amazing when they bake. I always ask for her recipe."
  ];
  ...
  // Split into 1-2 pages (simpler than Hebrew)
  const numPages = Math.min(2, Math.ceil(sentences.length / 2));
  ...
  const questions: Question[] = [
    {
      ...
      options: ["Animals", "People", "Food", "School"],
      correctIndex: 1
    },
    {
      question: "How many sentences are in the text?",
      options: ["2-3", "4-5", "6-7", "More than 7"],
      correctIndex: sentences.length <= 3 ? 0 : sentences.length <= 5 ? 1 : 2
    }
  ];
...
export const EnglishGame = () => {
  ...
  const finishGame = () => {
    ...
    const timeBonus = Math.max(0, 30 - Math.floor(timeTaken / 30));
      ...
      {phase === "reading" && round && (
        <Card className="animate-fade-in">
          ...
          <CardContent className="space-y-6">
            <div 
              ref={containerRef}
              className="min-h-[250px] p-6 bg-muted/50 rounded-xl text-lg leading-relaxed cursor-grab transition-transform"
              dir="ltr"
